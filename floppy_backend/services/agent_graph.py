...
class AgentGraphBuilder:
    ...
    def _normalize_request(self, state: AgentState) -> dict[str, Any]:
        ...
        return {"normalized_request": normalized, "cache_key": cache_key}
    ...
    def _play_asset(self, state: AgentState) -> dict[str, Any]:
        ...
        return {"action": "play_asset", "asset": result.asset, "job_id": None, "reasons": result.reasons}

    def _no_match(self, state: AgentState) -> dict[str, Any]:
        return {"action": "no_match", "asset": None, "job_id": None, "reasons": ["资产库未命中，且未授权生成"]}

    def _create_generation_job(self, state: AgentState) -> dict[str, Any]:
        req = state["request"]
        # Budget check is enforced inside GenerationService
        ...
        return {"action": "generate_job", "asset": None, "job_id": response.job_id, "reasons": ["资产库未命中，已创建生成任务"]}
    ...
    def _build(self) -> Any:
        ...
        graph.add_conditional_edges(
            "search_assets",
            self._decide_route,
            {"play_asset": "play_asset", "no_match": "no_match", "generate": "create_generation_job", "error_end": END},
        )
