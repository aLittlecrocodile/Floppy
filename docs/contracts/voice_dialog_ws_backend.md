...
```
App麦克风 → /voice/ws → 火山流式ASR → Hermes voice-dialog Skill → MiniMax流式TTS → App扬声器
                                                       └─ 明确音频请求 → sleep-audio workflow → audio_asset / audio_job
```

ASR 定稿后由 Hermes `floppy-voice-dialog` 判断是聊天、追问，还是进入音频 workflow。支持 barge-in（新最终识别取消进行中的回复）。
...
- `floppy_backend/services/voice_dialog_router.py` — Hermes `floppy-voice-dialog` 路由适配器
...
```bash
# Hermes Agent API Server（voice-dialog + sleep-audio Skill）
export FLOPPY_HERMES_BASE_URL="http://127.0.0.1:8642"
export FLOPPY_HERMES_API_KEY="<hermes_or_oneapi_key>"
export FLOPPY_HERMES_MODEL="DeepSeek-V4-Flash"
# 火山流式 ASR
...
- 密钥写在客户端，一旦泄露等于人人可用，且每次对话都会真实消耗 **火山 ASR + Hermes/LLM + MiniMax TTS** 三套付费 API。
